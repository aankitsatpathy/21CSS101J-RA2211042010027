...
def modify_data():
  model_name = input("Enter criminal name to modify: ")#user input taken
  field_name = input("Enter field name to modify (Date of charge, date of arrest,time of arrest, Charges, date of release, Review of stay): ")#user input taken
  new_value = input("Enter new value: ")#user input taken
  sql = f"UPDATE specifications SET {field_name} = %s WHERE Criminal_Name = %s"
